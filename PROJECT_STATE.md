...
* Architectural intent — 28 workflows, 43 repositories, 28 route modules, 8 transition maps (Request, Job, Invoice, Lease, RentalApplication, Rfp, RfpQuote, CashflowPlan)
...
| Repositories | 43 | src/repositories/ — derived (excl. index.ts) |
...
See [EPIC_HISTORY.md](EPIC_HISTORY.md) for full narrative. Summary: post-`REQUEST_CREATED` async handler scores contractors and writes suggestions + budget hint to the request. Manager sees a read-only hint panel on request detail. 1073 tests · 105 migrations.
