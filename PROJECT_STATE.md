...
* Architectural intent — 28 workflows, 52 repositories, 28 route modules, 8 transition maps (Request, Job, Invoice, Lease, RentalApplication, Rfp, RfpQuote, CashflowPlan)
...
See [EPIC_HISTORY.md](EPIC_HISTORY.md) for full narrative. Summary: post-`REQUEST_CREATED` async handler scores contractors and writes suggestions + budget hint to the request. Manager sees a read-only hint panel on request detail. 1073 tests · 124 migrations.
