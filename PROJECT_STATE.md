...
> - **Pre-GA security — sandbox/branch policy hardened 2026-06-23.** `registerSandboxRoutes` now no-ops unless `SANDBOX_MODE=true` (routes don't exist off-sandbox — 404 at the router, not just the per-handler 403). Added a production boot guard: `SANDBOX_MODE=true` is fatal on any branch except `sandbox`, so demo seeding can never activate on the `main` deployment. (`sandbox` stays in the branch allowlist — it's the legitimate beta-sandbox Render service.) **Still deferred by design (need infra / staging tuning):** durable (Redis) rate limiting, CSP header (tune as report-only in staging first), audit logging.
