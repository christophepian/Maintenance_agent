...
> - **Pre-GA security — sandbox/branch policy hardened 2026-06-23.** `registerSandboxRoutes` now no-ops unless `SANDBOX_MODE=true` (routes don't exist off-sandbox — 404 at the router, not just the per-handler 403). Added a production boot guard: `SANDBOX_MODE=true` is fatal on any branch except `sandbox`, so demo seeding can never activate on the `main` deployment. (`sandbox` stays in the branch allowlist — it's the legitimate beta-sandbox Render service.) CSP shipped as **report-only** (`Content-Security-Policy-Report-Only` in `next.config.js`) — non-enforcing, surfaces violations to map real origins before tightening. Rate limiting consolidated into one shared module (`apps/api/src/http/rateLimiter.ts`) — the four duplicated in-memory limiters (rental / capture / tenant-conversation / triage) now share a single namespaced store with a documented **Redis extension point** (the single seam to swap for durable, multi-instance limits). **Still deferred by design (need infra):** the actual Redis backend (needs a Redis instance + sync→async call-site change), audit logging; and flipping CSP from report-only to enforcing after a clean staging pass.
