...
🔒 **Architecture/scaling audit + multi-tenant isolation hardening (2026-06-30).** A read-only architecture/scaling/UX audit (full findings in `docs/PERFORMANCE_AUDIT_2026-06-30.md`, lands with PR #1) then three vertical slices, each on its own branch (all **open PRs, none merged** — review/merge order matters):
> **PR #1 `perf/portfolio-financials-concurrency` — portfolio reporting perf.** `getPortfolioMonthlyBreakdown`'s serial 12-iteration `for await` → bounded-concurrent (limit 3); `getBuildingFinancials` 3×→1× active-unit-id fetch; `getPortfolioSummary`'s **unbounded** `Promise.allSettled` → bounded `mapWithConcurrency` (limit 4, `src/utils/concurrency.ts`). No financial figure changes — gated by `portfolioBreakdown.characterization.test.ts` (totals == Σ per-building rows; `monthlyBreakdown[m]` == standalone per-month summary). Deep cross-building batch deferred (snapshot cache returns a "lite" DTO — totals depend on that shape; the characterization test is its gate).
> **PR #2 `security/tenant-isolation-idor` — 9 verified cross-tenant IDOR leaks.** Three parallel auditors; every "exposed" claim verified against code (3 false alarms downgraded). **Rental apps (6, worst — applicant PII + cross-org mutation):** `getApplication`/`adjustEvaluation`/`overrideDisqualification` + repo finders now require `orgId` (added `findApplicationByIdAndOrg`; kept unscoped `findApplicationById` for the internal submission workflow). **Invoices (2, write-IDOR):** `PATCH /invoices/:id` org-guarded before mutation; `swapInvoiceParties` checks org **before** the swap (was a TOCTOU). **Condition photos (1):** `GET /condition-report-photos/:photoId` scopes via `item→report.orgId`. Test `tenantIsolationIdor.test.ts` (9). Scoping rubric: `where:{id,orgId}` (direct/relation) or `resolveXOrg`+`assertOrgScope` (`src/governance/orgScope.ts`). Pre-pilot → no real data was exposed.
> **PR #3 `security/isolation-hardening-wsA` — defense-in-depth + plan.** Scoped the fragile mutation repos at the DB (`updateMortgage`/`deleteMortgage`, `updateBillingPeriod`, `deleteCostEntry` — were route-pre-checked only); test `scopedMutationsIdor.test.ts`. Lands the full plan [docs/TENANT_ISOLATION_HARDENING_SCOPE.md](docs/TENANT_ISOLATION_HARDENING_SCOPE.md) (Workstream B = `orgId` denormalization on 21 child models, phased).
>
> Side effect: an accidental concurrent commit had bundled the security files with an unrelated inventory change — split out to `feat/unit-rent-charges` (clean, separate). Full suite throughout: all pass **except** the pre-existing `healthContract.test.ts:56` failure (reproduces on `main` with all session changes stashed).
>
> **Open backlog (2026-06-30):** **P0** merge the 3 PRs (esp. #2). **P1** isolation Workstream B (orgId denormalization — Phase 1 financial children → Phase 2 PII/IDOR-backstop → Phase 3 remainder; + `G24` ratchet + parametrized isolation test); async OCR boundary (audit Tier-1 #2 — `routes/rentalApplications.ts:154` scans synchronously; decided arch = in-process drain like email, not Redis). **P2** money→integer cents (audit #4 — 14 `…Chf Float` valuation/mortgage fields feed NPV/DSCR/LTV); portfolio financials deep batch (audit #1 deferred). **P3** frontend (`buildings/[id].js` 3286-line waterfall → Promise.all + tab split; 251 boilerplate proxies → `[...slug].js` catch-all). **Hygiene:** `healthContract` red on `main` (pre-existing drift) — fix /health shape or re-baseline.
...
⚡ **Portfolio reporting perf — bounded concurrency + query dedupe (2026-06-30).** First slice of a fresh architecture/scaling audit (full findings + backlog: [docs/PERFORMANCE_AUDIT_2026-06-30.md](docs/PERFORMANCE_AUDIT_2026-06-30.md)). Targets the portfolio-financials fan-out behind owner/portfolio reporting, on branch `perf/portfolio-financials-concurrency` (PR #1) — **no financial figure changes**, guarded by a new characterization test. **Layer 0:** `portfolioBreakdown.characterization.test.ts` (seeds 2 buildings; pins totals == Σ per-building rows and `monthlyBreakdown[m]` == an independent per-month summary) + `src/utils/concurrency.ts` `mapWithConcurrency` (order-preserving, bounded) + unit tests. **Layer 1:** `getPortfolioMonthlyBreakdown`'s 12-iteration **serial** `for await` loop → bounded-concurrent (limit 3); formulas untouched. **Layer 2:** `getBuildingFinancials` fetched the active unit-id list **3× per building** → now once (threaded into `getReceivables`/`getPayables`); `getPortfolioSummary`'s **unbounded** `Promise.allSettled` over every building → bounded `mapWithConcurrency` (limit 4), preventing connection-pool saturation on large portfolios. `tsc` clean; financials + characterization + concurrency suites green; full suite 1166 passed (the lone `healthContract` failure is the known **pre-existing** main drift, reproduced with the change stashed). **Deferred (own review cycle):** the deep cross-building batch of the *fresh-compute* path — the snapshot cache returns a deliberately "lite" DTO, so portfolio totals depend on that shape; the characterization test is the gate. **Recommended next:** Tier-1 #2 async OCR boundary (`routes/rentalApplications.ts:154` scans synchronously in the request path).
