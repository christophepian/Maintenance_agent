...
* Architectural intent — 28 workflows, 46 repositories, 28 route modules, 8 transition maps (Request, Job, Invoice, Lease, RentalApplication, Rfp, RfpQuote, CashflowPlan)
...
| Field | Value | Source |
|-------|-------|--------|
| Models | 83 | prisma/schema.prisma — derived |
| Enums | 74 | prisma/schema.prisma — derived |
| Migrations | 109 | prisma/migrations/ — derived |
| Workflows | 32 | src/workflows/ — derived |
| Repositories | 46 | src/repositories/ — derived (excl. index.ts) |
...
| Tests | 74 suites · 1085 tests | jest — derived |
...
| Last manual review | 2026-06-19 | human |
...
See [EPIC_HISTORY.md](EPIC_HISTORY.md) for full narrative. Summary: post-`REQUEST_CREATED` async handler scores contractors and writes suggestions + budget hint to the request. Manager sees a read-only hint panel on request detail. 1073 tests · 109 migrations.
