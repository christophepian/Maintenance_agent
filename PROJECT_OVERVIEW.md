...
| Audit / open issues | [docs/AUDIT.md](docs/AUDIT.md) | 105 findings, 102 resolved, 3 open |
...
| Frontend page inventory | [docs/FRONTEND_INVENTORY.md](docs/FRONTEND_INVENTORY.md) | 104 UI pages (generated — `npm run inventory`) |
...
- Next.js 16 + React 19, Pages Router. `apps/web/pages/api/` proxies to backend via `proxyToBackend()` (289 proxy files; see Proxy conformance in PROJECT_STATE.md §Document Integrity).
...
- PostgreSQL 16 via Docker. Prisma ORM. 99 models · 82 enums · 137 migrations.
...
1. **Semantic tokens** — 35 CSS custom properties in `globals.css @theme {}` (brand, destructive, success, warning, muted, foreground, surface + table variants). **Target:** all color comes from these tokens; no hardcoded `slate-*`, `bg-white`, or `style={{}}` in JSX or `@layer components`. **Current compliance (audited 2026-06-23):** the `@layer components` layer is clean, but ~25 JSX files still carry ~67 raw `slate-*`/`bg-white` occurrences and ~29 files use inline `style={{}}` (concentrated in `login.js`, `index.js`, `HubBar.js`, `PortfolioCanvasChart.jsx`, `ImportedStatementsPanel.js`) — true exceptions must be marked `/* no-token: <reason> */`; the rest are debt to migrate. **New tokens added 2026-05-30:** `--color-foreground` (slate-900), `--color-foreground-dim` (slate-400), `--color-surface-subtle` (slate-50), `--color-surface-divider` (slate-100).
...
- `Request` has a **direct `orgId`** column (DT-114) — scope with `requestOrgScopeWhere(orgId)`, not the unit → building chain. `Request.unitId` is **nullable** (building-level COMPLAINT / ADMINISTRATIVE requests have no unit)
...
### Audit / debugging
1. Read [docs/AUDIT.md](docs/AUDIT.md) — check if any open findings (3 open) apply
...
**Graduated-autonomy confidence tiers + golden-file corpus + analyze-wait UX (2026-07-30):** Systematizing multi-format régie ingestion. **`computeConfidenceTier`** (importedStatementService, pure + 10 tests) collapses the FORMAT-AGNOSTIC invariant results — reconciliation, cross-statement result, sanity flags, OCR floor — into **GREEN / AMBER / RED** + reasons, exposed as `confidence{tier,reasons}` on the statement DTO. **`commitPackage` auto-approves only GREEN**; AMBER (ties but unverified / a warning) and RED (a failed invariant) are held `PENDING_REVIEW` — safe-by-default so questionable figures never post to client reporting unattended. The **review page** shows the verdict banner + reasons. Fixed a latent bug the tier surfaced: CSV/deterministic ingest stamped `ocrConfidence: 1` (a "no OCR" sentinel read as 1% → would have made every CSV + régie-package import AMBER) → now `null`. **Golden-file harness** (`apps/api/src/__tests__/extractionGolden.test.ts` + `fixtures/extractions/*.json` + `scripts/capture-extraction-fixture.js`, refresh with `UPDATE_GOLDENS=1`) replays each conquered format's canonical CSVs through the same invariant pipeline so an extractor change can't silently regress it. **UX:** shared `AnalyzeProgress` component replaces the level-0 "Analyzing…" spinner on both the building panel (`PackageOnboardingPanel`) and the onboarding wizard — eased progress bar (asymptotes to 92%, snaps to 100% on response), elapsed-driven stage checklist, countdown, rotating Swiss real-estate trivia, and an optional *beatable* tic-tac-toe (`TicTacToe.jsx`); **`/analyze-preview`** renders it with no upload / no API cost for review (throwaway page — safe to delete). Fuller strategy (more anchor subtotals, format registry + fingerprinting, correction feedback loop, propagating real vision confidence, i18n the review page) in the `owner-reporting-portfolio-dashboard` memory.

**Import extraction confidence + cost/resilience hardening (2026-07-29 → 07-30):** trust-and-cost pass on régie statement ingestion, on top of the reconciliation gate + confidence layers. **(1) COA code-collision fix** — `matchAccount` (importedStatementService) trusted an exact chart-of-accounts *code* hit unconditionally, but régie charts reuse our canonical Swiss-COA numbers for *different* accounts (a régie's `4500 Electricité` collided with canonical `4500 Property Management Fee`, `4600 Honoraires de gestion` → `4600 Property Tax`, `4100 Contrats d'entretien` → `4100 Mortgage Interest`), mislabelling **and** miscategorising lines. Now an exact-code hit is trusted only when the name is also consistent; otherwise it falls through to name/Claude matching. **Régie statements now display their OWN account name/code** (`aggregateImportedPnl` prefers `rawAccountName`/`rawAccountCode`), categorised from the raw name — canonical link reserved for the manager's explicit `costCategory` override. **(2) Extraction cache** — new **`ExtractionCache`** model (migrations `20260729120000` interim + `20260729130000`; content-hash + `EXTRACTOR_VERSION` key, org-scoped) caches expensive vision extractions across BOTH the single-statement path (`scanDocument`) and the régie-package wizard (`extractPackageFromPdf`) → re-uploading a byte-identical PDF skips the ~$0.45 vision pass; cache is **non-fatal** (a cache error degrades to a normal extraction, never fails the upload) and the package path stores the extracted CSVs so they're inspectable via SQL. **(3) Skip paid matching on income statements** — reference-only + display raw names ⇒ free exact/fuzzy still links, else UNMATCHED; balance sheets keep full matching (they post opening balances to the ledger). **(4) Anthropic resilience** — shared client `maxRetries: 6` (SDK exponential backoff) rides out transient `429/5xx/529 overloaded_error`; onboarding analyze surfaces an overload as **503 `EXTRACTION_OVERLOADED`** ("retry in a moment"), not a 502. **Gotcha documented:** imported balance-sheet opening balances post dated to fiscal-year-end (e.g. `2024-12-31`), but `/manager/finance/ledger` defaults to the **last 30 days** — historical imports look "missing" until the date filter is widened (UX gap to address). Working notes in the `owner-reporting-portfolio-dashboard` memory.

**Régie Package Onboarding + Imported-Actuals Reporting (2026-07-09 → 07-11, PRs #34–#56):** upload a régie's year-end package (rent roll, general ledger, balance sheet, income statement — any mix) on a building → detect + reconcile → one commit. Rent roll → Units/Tenants/Leases (merge-not-duplicate via unit-number-or-floor+rent matching; `activate` vs `snapshot` billing); general ledger → reference-only, vendor-deduplicated contractor invoices attributed per unit (feeds a top-vendors card, never posted to the ledger); balance sheet + income statement → ingested and approved. Building reporting substitutes the **approved imported income statement** for a covered fiscal year (`BuildingFinancialsDTO.source`), with sign-normalised P&L, per-unit contractual-rent fallback, and monthly/canvas points that distribute the annual statement over real invoice dates (even income + invoice-dated expenses, reconciling to the statement). Performance canvas redesigned to a stacked income bar (Expenses base + NOI margin) with health KPIs in the hover tooltip and brand-accent tokens (light + dark). New services: `rentRollMapper`, `regieLedgerMapper`, `packageDetector`, `buildingOnboardingService`, `invoiceOnboardingService`, `packageOnboardingService`. Full narrative in [EPIC_HISTORY.md](EPIC_HISTORY.md); working notes in the `project_csv_import` memory.

71 suites · 1048 tests (100% passing) · 0 TS errors · 102/105 audit findings resolved (3 open). Strategy Engine & Capture Hardening epic complete (3-phase strategy engine with 56 tests, Azure OCR activation, invoice source-file serving). Responsive polish pass complete: dual-render pattern (F-UI9) applied to 18+ pages across all 4 personas. Audit remediation pass: prisma-in-routes eliminated (userRepository/requestRepository/rfpRepository), workflowCoverage tests added, openapi.yaml updated with 3 missing routes, schema drift bug fixed (AUTO_APPROVED migration backfilled). Auth hardening: contractor IDOR fixed via `resolveContractorId()` (identity derived from JWT email, never query param); login page redesigned (magic-link + password tabs, first-time `/set-password` flow, `/reset-password` for recovery). QR invoice capture working end-to-end (2026-05-12): six bugs fixed — missing POST proxy, LAN-IP QR URL, phone routes behind auth wall, PDF iframe auth in overlay and detail page, source image auth. See PROJECT_STATE.md §QR invoice capture. Supabase RLS hardening (2026-05-13): RLS enabled on all 72 public tables via migration `20260513000000_enable_rls_public_tables`; no policies added (intentional — app never uses PostgREST directly; service_role bypasses RLS; direct PostgREST access now blocked for anon/authenticated roles). Notification fix (2026-05-14): notifications were silently returning 0 rows because the Supabase JWT `userId` never matched `User.id` — fixed by resolving `User.id` from `supabaseId` (then email fallback) after token verification. Tenant portal consolidation (2026-05-14): `/tenant/leases` and `/tenant/invoices` merged into `/tenant/myhome`. Swiss QR bill fix: SPC payload now complies with SIX spec §4.2. Manager dashboard: READY_TO_SIGN leases excluded from attention feed. Route-layer Prisma cleanup (2026-05-25): 6 remaining direct Prisma calls eliminated across `inventory.ts` (×3 — owner full-detail fetch, owner update, sync-buildings), `contractor.ts` (×1 — resolveContractorId lookup), `auth.ts` (×2 — dev tenant-list and tenant-login); 4 new repository functions added (`findOrgOwnerByIdFull`, `updateOwnerUser`, `syncAllBuildingsForOwner`, `findContractorByOrgAndEmail`, `listTenantsForDevImpersonation`, `findTenantForDevLogin`). workflowCoverage suite expanded: 4 new workflow tests added (`analyseClaimWorkflow`, `processTurnWorkflow`, `strategyProfileWorkflow`, `evaluateRecommendationWorkflow`) — now 21 tests in suite, 1048 total.

**Ancillary Costs v3 + Reporting recognition & i18n (2026-06-23):** Incoming invoices now split by **nature** at the review gate — `Invoice.costNature` (CHARGE = recoverable Nebenkosten → building cost pool, ventilated to units; DIRECT = repair/maintenance/capex → ledger/billing) + `ancillaryCategoryId` (migrations `20260623020000`, `…030000`). CHARGE invoices bridge to the cost pool on approval; building/unit reporting reads the pool (`recoverableAncillaryCents`, per-unit `apportionedChargesCents`), de-duped vs ledger by source invoice. Fixes: empty-string FK save bug; billing-period boundary lookup (`findBillingPeriodOverlappingWindow`); ledger attribution backfill when an invoice is classified after posting. **Recognition clarity:** the collection rate is billing-period-scoped (paid/invoiced) so a monthly view can read 0% even when cash was collected (arrears timing — not a bug); KPIs relabeled "Rent Collected"→"Cash received", "Collection Rate"→"On-time collection". **Income fields renamed end-to-end** — `earnedIncomeCents→collectedIncomeCents` (cash), `projectedIncomeCents→accruedIncomeCents` (accrual from lease terms) — including the 3 snapshot DB columns (migration `…030000`, data preserved). Building/owner/unit reporting fully i18n'd (EN + Swiss-FR) with locale-aware dates + dark-aware hero gradients. See SCHEMA_REFERENCE §Schema Gotchas.
...
**Render config (2026-05-09):** Service now deploys from `main` branch with `autoDeploy: true` — every push to main triggers a build automatically. Build command corrected to `cd apps/api && npm ci && npx prisma generate --schema ./prisma/schema.prisma && rm -rf dist && npm run build`. Boot-time guard added: server refuses to start in production if `RENDER_GIT_BRANCH` is not `main`. Public listings seeded (7 units across 3 buildings, `isListedPublicly: true`). App rebranded to Propfolio in sidebar nav. Mobile nav labels synced with desktop (Properties, Contacts).
