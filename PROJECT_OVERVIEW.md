...
**i18n (bilingual EN/FR — live as of 2026-05-05):** All UI strings extracted through `next-i18next`. Five namespaces: `common`, `manager`, `owner`, `contractor`, `tenant`. Tab labels rendered via `t(\`ns:section.tabs.\${key}\`)` template literals — runtime keys must exist in locale JSON. Critical dev rule: `reloadOnPrerender: true` is set in `next-i18next.config.js` so locale file changes are picked up without a server restart. Audit scripts: `scripts/i18n-audit-missing.py` (static key coverage), `scripts/i18n-audit-tabs.py` (template-literal tab key coverage).
