...
66 suites · 1009 tests (100% passing) · 0 TS errors · 91/94 audit findings resolved. Strategy Engine & Capture Hardening epic complete (3-phase strategy engine with 56 tests, Azure OCR activation, invoice source-file serving). Responsive polish pass complete: dual-render pattern (F-UI9) applied to 18+ pages across all 4 personas. Audit remediation pass: prisma-in-routes eliminated (userRepository/requestRepository/rfpRepository), workflowCoverage tests added, openapi.yaml updated with 3 missing routes, schema drift bug fixed (AUTO_APPROVED migration backfilled). Auth hardening: contractor IDOR fixed via `resolveContractorId()` (identity derived from JWT email, never query param); login page redesigned (magic-link + password tabs, first-time `/set-password` flow, `/reset-password` for recovery). QR invoice capture working end-to-end (2026-05-12): six bugs fixed — missing POST proxy, LAN-IP QR URL, phone routes behind auth wall, PDF iframe auth in overlay and detail page, source image auth. See PROJECT_STATE.md §QR invoice capture. Supabase RLS hardening (2026-05-13): RLS enabled on all 72 public tables via migration `20260513000000_enable_rls_public_tables`; no policies added (intentional — app never uses PostgREST directly; service_role bypasses RLS; direct PostgREST access now blocked for anon/authenticated roles). Resolves all Supabase linter `rls_disabled_in_public` and `sensitive_columns_exposed` findings. Notification fix (2026-05-14): notifications were silently returning 0 rows because the Supabase JWT `userId` (= Supabase `sub` UUID) never matched `User.id` (Prisma UUID) — `prismaUserId` was documented in app_metadata but never written. Fixed in `server.ts` by resolving `User.id` from `supabaseId` (then email fallback) after token verification, cached per process. No backfill script required — email fallback works for all existing users immediately. Tenant portal consolidation (2026-05-14): `/tenant/leases` and `/tenant/invoices` list pages merged into `/tenant/myhome` — single page shows lease cards + invoice summary + invoice list; sidebar "My Home" nav item updated; old routes issue permanent redirects; locale detection disabled (`localeDetection: false`) so French-browser users no longer receive FR strings on EN paths. Swiss QR bill fix: SPC payload now complies with SIX spec §4.2 — combined address type K requires empty elements 9 & 10 (postal code / city), with postal+city combined into addressLine2; validator guard added. Staging IBAN seeded via `apps/api/scripts/seed-staging-iban.sql` (QR-IBAN CH5631000123456789012). Manager dashboard: READY_TO_SIGN leases excluded from attention feed (no manager action pending once sent for signature).
