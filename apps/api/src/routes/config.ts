...
import { findBuildingByIdAndOrg, findUnitByIdAndOrg } from "../repositories/inventoryRepository";
...
export function registerConfigRoutes(router: Router) {
  ...
  router.put("/org-config", withAuthRequired(async ({ req, res, prisma, orgId }) => {
    try {
      ...
      if (input.autoApproveLimit === undefined && input.autoLegalRouting === undefined && input.invoiceLeadTimeDays === undefined && input.mode === undefined) {
      ...
      const updated = await updateOrgConfig(prisma, orgId, {
        ...
        invoiceLeadTimeDays: input.invoiceLeadTimeDays,
