...
describe("computeRenovationNoiAdjustments", () => {
  ...
  it("vacancy (days) adds a one-time lost-rent cost in the work year (Invest), pushed for Defer, none for Neglect", () => {
    // 30.44 days × 2'000/mo ≈ 1 month ≈ 2'000 lost rent
    const RENO_VAC = { ...RENO, unitId: "u1", vacancyDays: 30.44, unitMonthlyRentChf: 2_000 };
    const { investNoiAdj, deferNoiAdj, neglectNoiAdj } =
      computeRenovationNoiAdjustments([RENO_VAC], fromYear, toYear, deferYears);
    expect(investNoiAdj.get(2026)).toBe(-1_200 - 2_000); // risk + ~1mo vacancy in work year
    expect(investNoiAdj.get(2027)).toBe(6_000);          // uplift unaffected
    expect(deferNoiAdj.get(2029)).toBe(-1_200 - 2_000);  // vacancy at the deferred work year (2029)
  ...
  it("vacancy is valued once per unit when multiple assets share a unit", () => {
    const a = { ...RENO, assetId: "a1", unitId: "u1", vacancyDays: 30.44, unitMonthlyRentChf: 2_000 };
    const b = { ...RENO, assetId: "a2", unitId: "u1", vacancyDays: 30.44, unitMonthlyRentChf: 2_000 };
    const { investNoiAdj } = computeRenovationNoiAdjustments([a, b], fromYear, toYear, deferYears);
    // two assets in one unit → risk doubles (-2'400) but vacancy counts once (-2'000), not twice
    expect(investNoiAdj.get(2026)).toBe(-2_400 - 2_000);
  });
});
