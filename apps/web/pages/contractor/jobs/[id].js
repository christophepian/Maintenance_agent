    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <h2 className="text-base font-semibold text-gray-900">Rate the tenant</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          {CRITERIA.map((c) => (
            <div key={c.key}>
              <p className="mb-1 text-sm font-medium text-gray-700">{c.label}</p>
...
function SlotPanel({ jobId, contractorId, jobStatus, onRefresh }) {
  ...
  const SLOT_COLORS = {
    PROPOSED: "border-yellow-200 bg-yellow-50 text-yellow-800",
    ...
    <Panel title="📅 Appointment Slots">
      {slots.length > 0 && (
        <div className="space-y-2 mb-3">
          {slots.map((s) => (
            <div key={s.id} className={`flex items-center justify-between rounded-lg border px-3 py-2 text-sm ${SLOT_COLORS[s.status] || "bg-gray-50 border-gray-200"}`}>
      ...
      {slots.length === 0 && !showForm && canPropose && (
        <p className="text-sm text-gray-500 mb-3">No slots proposed yet. Propose times for the tenant to choose from.</p>
      )}
        ...
        <>
          {!showForm ? (
            <button
              ...
              className="mt-1 rounded-md bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-indigo-700"
            >
            ...
            <form onSubmit={handleSubmit} className="mt-2 space-y-3">
              ...
              {rows.map((r, i) => (
                <div key={i} className="flex flex-wrap items-center gap-2">
                  ...
                  />
                  <select value={r.startHour} onChange={(e) => updateRow(i, "startHour", e.target.value)} className="rounded border border-gray-200 px-2 py-1 text-xs">
                    {Array.from({ length: 13 }, (_, h) => h + 7).map((h) => (
                      <option key={h} value={String(h).padStart(2, "0")}>{String(h).padStart(2, "0")}h</option>
                    ))}
                  </select>
                  <select value={r.startMin} onChange={(e) => updateRow(i, "startMin", e.target.value)} className="rounded border border-gray-200 px-2 py-1 text-xs">
                    {["00", "15", "30", "45"].map((m) => <option key={m} value={m}>{m}</option>)}
                  </select>
                  <select value={r.durationH} onChange={(e) => updateRow(i, "durationH", e.target.value)} className="rounded border border-gray-200 px-2 py-1 text-xs">
                    {["1", "1.5", "2", "3", "4"].map((d) => <option key={d} value={d}>{d}h</option>)}
                  </select>
                  {rows.length > 1 && (
                    <button type="button" onClick={() => removeRow(i)} className="text-xs text-red-500 hover:text-red-700">✕</button>
                  )}
                </div>
              ))}
              {rows.length < 5 && (
              ...
              <div className="flex gap-2 pt-1">
                <button type="button" onClick={() => setShowForm(false)} className="rounded border border-gray-200 px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-50">Cancel</button>
                <button type="submit" disabled={submitting} className="rounded bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-indigo-700 disabled:opacity-50">
    ...
    return (
      <AppShell role="CONTRACTOR">
        <PageShell>
          ...
          <div className="error-banner mt-4">{error || "Job not found"}</div>
        </PageShell>
      </AppShell>
    );
  }
    ...
    <AppShell role="CONTRACTOR">
      <PageShell>
        ...
        <PageHeader
          ...
          actions={
            <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${STATUS_COLORS[job.status] || "bg-gray-100 text-gray-800"}`}>
        ...
        {!contractorId && (
          <div className="mb-4 rounded-lg border border-yellow-200 bg-yellow-50 px-4 py-3">
            <p className="mb-2 text-sm font-medium text-yellow-800">Select your contractor profile to use job actions:</p>
        ...
        <PageContent>
          {actionError && (
            <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-sm text-red-700">{actionError}</div>
          )}
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {acceptedSlot && (
              <Panel title="✅ Confirmed Appointment">
                <p className="text-sm text-indigo-800 font-medium">{fmtTime(acceptedSlot.startTime)} – {fmtTime(acceptedSlot.endTime)}</p>
          ...
          {job.status === "COMPLETED" && contractorRated && (
            <Panel bodyClassName="text-center py-6">
              <p className="text-indigo-800 mb-3 font-medium">This job is completed. Ready to submit an invoice?</p>
              <Link
          ...
          {job.status === "INVOICED" && (
            <Panel>
              <p className="text-purple-800 font-medium">✅ Invoice submitted for this job.</p>
