...
export default function ImportedStatementReviewPage() {
  ...
  // Sibling sections from the same batch
  ...
  // Load sibling sections once we know the uploadBatchId
  useEffect(() => {
    const batchId = statement?.uploadBatchId;
    if (!batchId) return;
    fetch(`/api/imported-statements/batch/${batchId}`, { headers: authHeaders() })
      .then((r) => r.json())
      .then((j) => setBatchSections(j.data?.statements ?? []))
      .catch(() => {});
  ...
  async function handleBalanceDelete(balanceId) {
    if (!window.confirm("Delete this balance row? This cannot be undone.")) return;
    setDeletingBalanceId(balanceId);
    try {
      const res = await fetch(
        `/api/imported-statements/${id}/balances/${balanceId}`,
        { method: "DELETE", headers: authHeaders() },
      );
      const json = await res.json().catch(() => null);
      if (!res.ok) throw new Error(json?.error?.message || "Failed to delete");
      setStatement(json.data);
      if (editingBalanceId === balanceId) setEditingBalanceId(null);
    } catch (e) {
  ...
  const siblingNav = batchSections.filter((sec) => sec.id !== id);
              ...
              {siblingNav.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {siblingNav.map((sec) => (
                    <Link
                      key={sec.id}
                      href={`/manager/finance/imports/${sec.id}`}
                      className={cn(
                    ...
                    >
                      ...
                      <span className="opacity-75">{sec.status.replace(/_/g, " ")}</span>
                    </Link>
                  ))}
                </div>
              )}
                          ...
                          <tfoot>
                            ...
                            <tr className="border-t border-slate-300 bg-slate-100 text-sm font-semibold">
                              ...
                              <td className={cn(
                                ...
                                {isNetIncomeCents < 0 ? "(" : ""}{formatChfCents(Math.abs(isNetIncomeCents))}{isNetIncomeCents < 0 ? ")" : ""}
                              </td>
                              <td colSpan={isPendingReview ? 3 : 2} className="px-3 py-2">
                                <span className={cn(
                                  "text-xs font-medium",
                                  ...
                                  {isNetIncomeCents >= 0 ? "Profit" : "Loss"}
                                </span>
                              </td>
                            </tr>
                          </tfoot>
