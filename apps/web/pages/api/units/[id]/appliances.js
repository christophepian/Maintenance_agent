...
export default async function handler(req, res) {
  try {
    ...
    if (typeof includeInactive === "string") params.set("includeInactive", includeInactive);
    const qs = params.toString() ? `?${params.toString()}` : "";

    if (req.method === "GET") {
      const r = await fetch(`${baseUrl}/units/${id}/appliances${qs}`, {
    ...
    if (req.method === "POST") {
      ...
      const r = await fetch(`${baseUrl}/units/${id}/appliances`, {
        method: "POST",
        headers: {
