 ...
 * - Panel is a white card with divider-separated sections (border-t border-surface-divider)
...
export function FilterPanelBody({ children }) {
  return (
    <div className="mb-4 rounded-xl border border-surface-border bg-surface p-4 shadow-sm">
      {children}
    </div>
  );
}
...
export function FilterSection({ title, first = false, children }) {
  return (
    <div className={cn(!first && "mt-4 border-t border-surface-divider pt-4")}>
      <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-foreground-dim">{title}</p>
      {children}
    </div>
  );
}
...
export function FilterSectionClear({ hasFilter, onClear }) {
  const { t } = useTranslation("common");
  return (
    <div className="mt-4 flex justify-end border-t border-surface-divider pt-3">
      <button
        onClick={onClear}
        disabled={!hasFilter}
        className={cn(
          ...
          hasFilter
            ? "text-muted hover:text-muted-dark cursor-pointer"
            : "text-foreground-dim cursor-default"
        )}
      >
    ...
    <div className={cn("flex flex-col gap-1", className)}>
      ...
      <div className="relative">
        <select
          value={value}
          onChange={onChange}
          className="w-full min-h-[36px] appearance-none rounded-lg border border-surface-border bg-surface pl-3 pr-8 py-2 leading-tight text-sm text-muted-dark focus:outline-none focus:ring-2 focus:ring-blue-400"
        >
          {children}
        </select>
        <span className="pointer-events-none absolute inset-y-0 right-2 flex items-center text-foreground-dim">
...
export function CheckboxGroupField({ label, options, value, onChange }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {options.length === 0 ? (
        <p className="text-xs text-foreground-dim italic">No options available</p>
      ) : (
        <div className="flex flex-wrap gap-x-5 gap-y-2 pt-0.5">
          {options.map((opt) => (
            <label key={opt} className="flex items-center gap-1.5 text-sm text-muted-dark cursor-pointer select-none">
              <input
                type="checkbox"
                checked={value.includes(opt)}
                onChange={() => toggle(opt)}
                className="h-4 w-4 rounded border-muted-ring text-blue-600 accent-blue-600 focus:ring-blue-500"
              />
              {opt}
            </label>
          ))}
  ...
  return (
    <div className={cn("flex flex-col gap-1", className)}>
      ...
      <input
        ...
        min={min ?? 1}
        className="w-full min-h-[36px] rounded-lg border border-surface-border bg-surface px-3 py-2 text-sm text-muted-dark placeholder:text-foreground-dim focus:outline-none focus:ring-2 focus:ring-blue-400 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
      />
    </div>
  );
}
...
export function SortPanelBody({ children }) {
  ...
  return (
    <div className="mb-4 rounded-xl border border-surface-border bg-surface p-4 shadow-sm">
      <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-foreground-dim">{t("filterPanel.sortBy")}</p>
    ...
    <button
      ...
      className={cn(
        ...
        active
          ...
          : "text-muted-text hover:bg-surface-subtle"
      )}
    >
      <span className="flex items-center gap-2">
        ...
        <span className={cn(
          ...
          active ? "border-blue-600 bg-blue-600" : "border-muted-ring bg-surface"
        )} aria-hidden="true" />
        {label}
      </span>
      {active && (
  ...
  return (
    <div className={cn("flex flex-col gap-1", className)}>
      ...
      <input
        ...
        value={value}
        onChange={onChange}
        className="h-9 w-full appearance-none rounded-lg border border-surface-border bg-surface px-3 py-2 leading-tight text-sm text-muted-dark focus:outline-none focus:ring-2 focus:ring-blue-400"
      />
    </div>
  );
}
